...
var Reveal = (function(){
	...
	function nextFragment() {
		...
		var presentSlide = document.querySelector(".reveal section.present");
		var currentSlideStep = parseInt(presentSlide.getAttribute("data-fragment-step") || "0", 10);
		var nextSlideStep = currentSlideStep + 1;
		var currentFragmentSelector = document.querySelector( VERTICAL_SLIDES_SELECTOR + '.present') ? VERTICAL_SLIDES_SELECTOR + '.present ' : HORIZONTAL_SLIDES_SELECTOR + '.present *[data-fragment-' + (nextSlideStep) + ']';
		var currentFragment = document.querySelector(currentFragmentSelector);
		if (!currentFragment) return false;
		var animations = currentFragment.getAttribute("data-fragment-" + nextSlideStep);
		if (animations) {
			...
			for (var i = 0; i < animations.length; i++) {
				...
				if (matches.length > 1) {
					...
					if (funcName === "copy-to") {
						...
						var from = currentFragment.cloneNode(true);
					...
					else if (funcName === "toggle-class") currentFragment.classList.toggle(content.trim());
					else if (funcName === "remove-class") currentFragment.classList.remove(content.trim());
					else if (funcName === "add-class") currentFragment.classList.add(content.trim());
					else if (funcName === "dispatch-event") dispatchEvent( content.trim(), { fragment: currentFragment } );
					else if (funcName === "change-content") currentFragment.innerHTML = content.trim();
				}
			}
		}
		presentSlide.setAttribute("data-fragment-step", nextSlideStep);
		dispatchEvent( 'fragmentshown', { fragment: currentFragment } );
		return true;
	}
	...
	function previousFragment() {
		var fragments = document.querySelectorAll(document.querySelector( VERTICAL_SLIDES_SELECTOR + '.present') ? VERTICAL_SLIDES_SELECTOR + '.present .fragment.visible' : HORIZONTAL_SLIDES_SELECTOR + '.present .fragment.visible');
		if (fragments.length) {
			var lastFragment = fragments[fragments.length - 1];
			var maxIndex = parseInt(lastFragment.getAttribute("data-fragment-index") || "10000000", 10);
			for (var i = fragments.length - 1; i >= 0; i--) {
				var currentIndex = parseInt(fragments[i].getAttribute("data-fragment-index") || "100000", 10);
				if (currentIndex >= maxIndex) {
					maxIndex = currentIndex;
					lastFragment = fragments[i];
				}
			}
			lastFragment.classList.remove( 'visible' );

			// Notify subscribers of the change
			dispatchEvent( 'fragmenthidden', { fragment: lastFragment } );
			return true;
		}
